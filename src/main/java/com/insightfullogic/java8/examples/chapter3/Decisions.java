...
public class Decisions {
	...
	public static void main(String[] args) {
		Decisions.Imperative staticClass = new Decisions.Imperative();
		List<Album> albumList = SampleData.getAlbum();
		Set<String> nationalities = new HashSet<>();
		for (Album album : albumList) {
			Set<String> nationality = staticClass.originsOfBands(album);
			for(String nation : nationality)
				nationalities.add(nation);
		}
		System.out.println(nationalities);
		for (Album album : albumList) {
			new Decisions().originsOfBands(album);
